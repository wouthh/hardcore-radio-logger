...
Playlist scans and backfill establish membership, not match correctness. Existing associations retain their confidence and review classification; uncertain or unknown-confidence matches remain under review when they reappear. Presence still refreshes provider metadata and last-seen time and clears missing suspicion. Confirmed associations retain normal removal handling, and excluded parents remain excluded. If a different Spotify ID would replace an existing track/playlist association, the snapshot import is rejected transactionally as an association conflict. In `run-once`, that conflict returns exit status 1 before reconciliation or either external sync stage, so later matching cannot bypass the refused snapshot.
